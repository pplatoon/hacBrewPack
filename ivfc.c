...
void ivfc_create_level6(filepath_t *dst_level_file, filepath_t *romfs_file, uint64_t *out_size)
{
    ...
    // Open files
    src_file = os_fopen(romfs_file->os_path, OS_MODE_READ);
    if (src_file == NULL)
    {
        fprintf(stderr, "Unable to open: %s", romfs_file->char_path);
        exit(EXIT_FAILURE);
    }
    dst_file = os_fopen(dst_level_file->os_path, OS_MODE_WRITE);
    if (dst_file == NULL)
    ...
    // Get source file size
    fseeko64(src_file, 0, SEEK_END);
    src_file_size = (uint64_t)ftello64(src_file);
    *out_size = src_file_size;
    ...
    while (ofs < src_file_size)
    {
        ...
        fwrite(buf, read_size, 1, dst_file);
        ofs += read_size;
    }

    uint64_t curr_offset = ftello64(dst_file);
